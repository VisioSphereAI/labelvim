...
from labelvim.widgets.list_widgets import CustomListViewWidget, CustomLabelListWidget
import sys
...
from labelvim.utils.config import ANNOTATION_TYPE, ANNOTATION_MODE
...
class LabelVim(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.DuplicateAnnotationBtn.clicked.connect(self.__duplicate_annotation)
    ...
    def __load_image(self, file_name, index):
        ...
        self.Display.update_pixmap(file_name)
        if self.save_dir:
        ...
        if self.save_dir:
            ...
            if self.annotaion_manager is None:
                ...
                self.annotaion_manager.update_basic_info(
                    ...
                    self.Display.pixmap.width(),
                    self.Display.pixmap.height()
                )
                self.annotaion_data = self.annotaion_manager.annotation
    ...
    def __duplicate_annotation(self):
        print("Duplicate Annotation")
        self.annotaion_mode = ANNOTATION_MODE.DUPLICATE
        print(f"Annotation Mode: {self.annotaion_mode}")
